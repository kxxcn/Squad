...
public class RecruitmentListFragment extends Fragment implements RecruitmentListContract.View, RecruitmentListContract.ItemClickListener, ListContract.OnDialogDismissed {
	...
	private RecruitmentListContract.Presenter mPresenter;
	...
	public static Fragment newInstance(User user) {
		RecruitmentListFragment fragment = new RecruitmentListFragment();
	...
	@Override
	public void setPresenter(RecruitmentListContract.Presenter presenter) {
	...
	public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
		...
		new RecruitmentListPresenter(this, DataRepository.getInstance(RemoteDataSource.getInstance(
				FirebaseAuth.getInstance(), FirebaseDatabase.getInstance().getReference())));

		mPresenter.setFiltering(Constants.ListsFilterType.RECRUITMENT_LIST);
	...
	@Override
	public void showRecruitmentList(List<Information> list) {
		mList = new ArrayList<>(0);
		this.mList = list;
		Collections.sort(list, new RecruitmentListFragment.Compare());
		rv_list.setAdapter(new RecruitmentListAdapter(getContext(), list, this));
	}
	...
	public void showSuccessfullyRequested() {
		UiThread.getInstance().post(new Runnable() {
			@Override
			public void run() {
				Toast.makeText(getContext(), getString(R.string.list_successfully_request_recruitment), Toast.LENGTH_SHORT).show();
			}
		});
	}
	...
	public void onClick(int position, int type) {
		if (type == MatchListAdapter.REQUEST) {
			mPresenter.onRequest(mList.get(position).getEmail(), getString(R.string.app_name), String.format(getString(R.string.list_request_recruitment),
					mUser.getTeam()), mUser.getUid(), mList.get(position).getDate().replace("-", ""), Constants.ListsFilterType.RECRUITMENT_LIST);
		} else if (type == MatchListAdapter.INFORMATION) {
			MatchDialog dialog = new MatchDialog(getActivity(), getContext(), MatchDialog.RECRUITMENT_LIST, mList.get(position));
